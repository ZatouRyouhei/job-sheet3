<template>
    <div class="business">
        ...
        <form @submit.prevent="handleSubmit(!v$.$invalid)" class="edit-form p-fluid">
            <label id="business-id-label" for="business-id" class="required">ID</label>
			<span id="business-id">
            ...
            <label id="name-label" for="name" class="required">業務（{{v$.name.maxLength.$params.max}}文字まで）</label>
...
<script lang="ts">
    ...
    export default defineComponent({
        setup() {
            ...
            const selectedBusiness = ref()  // 一覧で選択した業務情報
            ...
            const handleSubmit = (isFormValid: boolean) => {
                ...
                if (!isFormValid) {
                    ...
                    if (v$.value.name.maxLength.$invalid && submitted) {
                        addToast('業務は' + v$.value.name.maxLength.$params.max + '文字以内で入力してください。')
                    }
                } else {
                    // 入力エラーがない場合
                    loading.value = true
                    ...
                    axios.post(Constant.URL_BUSINESS_REGIST, businessObj).then(() => {
			...
			const delConfirm = (event:any) => {
				confirm.require({
					...
					accept: () => {
                        ...
                        axios.delete(Constant.URL_BUSINESS_DELETE + state.id).then((res) => {
            ...
            return({businessList, loading, delLoading, selectedBusiness, disabled, delDisabled, state, rules, v$, submitted, onRowSelect, handleSubmit, delConfirm, clearConfirm})
        }
    })
</script>
